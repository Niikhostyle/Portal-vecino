...
<div class="min-h-screen">
    <div class="mx-auto max-w-2xl px-4 py-8 sm:px-6 lg:px-8">
        <!-- Header -->
        <div class="mb-10">
            <h1 class="text-3xl font-bold tracking-tight text-slate-900">Editar Tipo de Solicitud</h1>
            <p class="mt-1 text-base text-slate-600">Actualiza la información del tipo de solicitud</p>
        </div>
        
        <!-- Form -->
        <div class="overflow-hidden rounded-2xl border border-slate-200 bg-white p-8 shadow-sm">
            <form method="POST" action="{{ route('admin.catalogo.update', $tipo->id) }}" class="space-y-6">
                @csrf
                @method('PUT')
                
                <div>
                    <label for="codigo" class="mb-2 block text-sm font-medium text-slate-700">Código</label>
                    <input type="text" id="codigo" value="{{ $tipo->codigo }}" disabled class="h-11 w-full rounded-lg border border-slate-300 bg-slate-50 px-4 py-2.5 text-sm text-slate-500">
                    <p class="mt-1.5 text-xs text-slate-500">El código no se puede modificar</p>
                </div>

                <div>
                    <label for="titulo" class="mb-2 block text-sm font-medium text-slate-700">Título</label>
                    <input type="text" id="titulo" name="titulo" value="{{ old('titulo', $tipo->titulo) }}" required class="h-11 w-full rounded-lg border border-slate-300 bg-white px-4 py-2.5 text-sm text-slate-900 placeholder:text-slate-500 focus:border-slate-900 focus:outline-none focus:ring-2 focus:ring-slate-900 focus:ring-offset-0 transition-colors @error('titulo') border-red-500 focus:border-red-500 focus:ring-red-500 @enderror">
                    @error('titulo')
                ...
                <div>
                    <label for="seccion" class="mb-2 block text-sm font-medium text-slate-700">Sección</label>
                    <input type="text" id="seccion" name="seccion" value="{{ old('seccion', $tipo->seccion) }}" required class="h-11 w-full rounded-lg border border-slate-300 bg-white px-4 py-2.5 text-sm text-slate-900 placeholder:text-slate-500 focus:border-slate-900 focus:outline-none focus:ring-2 focus:ring-slate-900 focus:ring-offset-0 transition-colors @error('seccion') border-red-500 focus:border-red-500 focus:ring-red-500 @enderror">
                    @error('seccion')
                        <p class="mt-1.5 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <textarea id="descripcion" name="descripcion" rows="4" class="w-full rounded-lg border border-slate-300 bg-white px-4 py-2.5 text-sm text-slate-900 placeholder:text-slate-500 focus:border-slate-900 focus:outline-none focus:ring-2 focus:ring-slate-900 focus:ring-offset-0 transition-colors resize-none @error('descripcion') border-red-500 focus:border-red-500 focus:ring-red-500 @enderror">{{ old('descripcion', $tipo->descripcion) }}</textarea>
                    @error('descripcion')
                        <p class="mt-1.5 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                @php
                    $camposSeleccionados = old('campos', $tipo->campos_requisitos);
                    $documentosRequeridos = old('documentos_requeridos', $tipo->documentos_requeridos);
                    if (empty($documentosRequeridos)) {
                        $documentosRequeridos = [''];
                    }
                    $tieneRecinto = in_array('recinto', $camposSeleccionados);
                    $tieneFechaHora = count(array_intersect(['fecha_inicio', 'fecha_fin', 'hora_inicio', 'hora_fin'], $camposSeleccionados)) === 4;
                    $usarHorariosDisponibles = old('usar_horarios_disponibles', $tipo->usar_horarios_disponibles ?? false);
                    $recintosIdsSeleccionados = old('recintos_ids', $tipo->recintos_ids ?? []);
                @endphp
                <div class="border-t border-slate-200 pt-6">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        @foreach(config('tramites.campos', []) as $key => $config)
                            <label class="flex items-center rounded-lg border border-slate-200 p-3 hover:bg-slate-50">
                                <input type="checkbox" name="campos[]" value="{{ $key }}" {{ in_array($key, $camposSeleccionados) ? 'checked' : '' }} class="h-4 w-4 rounded border-slate-300 text-slate-900 focus:ring-slate-900">
                                <span class="ml-2 text-sm text-slate-700">{{ $config['label'] }}</span>
                            </label>
                        @endforeach
                    </div>
                    <div id="bloqueRecintosEdit" class="mt-4 p-4 rounded-lg border border-slate-200 bg-slate-50 {{ $tieneRecinto ? '' : 'hidden' }}">
                        <label class="mb-2 block text-sm font-medium text-slate-700">Recintos disponibles para este trámite</label>
                        <p class="mb-3 text-xs text-slate-600">Selecciona los recintos que el ciudadano podrá elegir. Si no seleccionas ninguno, se usarán los filtros por código (deportivos/municipales).</p>
                        <select name="recintos_ids[]" multiple size="6" class="h-auto w-full rounded-lg border border-slate-300 bg-white px-4 py-2.5 text-sm text-slate-900 focus:border-slate-900 focus:outline-none focus:ring-2 focus:ring-slate-900 focus:ring-offset-0">
                            @foreach($recintos ?? [] as $recinto)
                                <option value="{{ $recinto->id }}" {{ in_array($recinto->id, $recintosIdsSeleccionados) ? 'selected' : '' }}>
                    ...
                    <div id="bloqueUsarHorarios" class="mt-4 p-4 rounded-lg border border-blue-200 bg-blue-50 {{ ($tieneRecinto && $tieneFechaHora) ? '' : 'hidden' }}">
                        <label class="flex items-start gap-3 cursor-pointer">
                            <input type="checkbox" name="usar_horarios_disponibles" value="1" {{ $usarHorariosDisponibles ? 'checked' : '' }} class="mt-1 h-4 w-4 rounded border-slate-300 text-slate-900 focus:ring-slate-900">
                            <div>
                    ...
                    document.querySelectorAll('input[name="campos[]"]').forEach(cb => {
                        cb.addEventListener('change', function() {
                            ...
                            const bloqueRecintos = document.getElementById('bloqueRecintosEdit');
                            const bloqueHorarios = document.getElementById('bloqueUsarHorarios');
                ...
                <div class="flex items-center">
                    <input type="checkbox" name="requiere_adjuntos" value="1" id="requiere_adjuntos" {{ old('requiere_adjuntos', $tipo->requiere_adjuntos) ? 'checked' : '' }} class="h-4 w-4 rounded border-slate-300 text-slate-900 focus:ring-slate-900">
                    <label for="requiere_adjuntos" class="ml-2 block text-sm text-slate-700">
                        Permitir adjuntar documentos
                    </label>
                </div>

                <div class="border-t border-slate-200 pt-6" x-data="{ docs: {{ json_encode($documentosRequeridos) }} }">
                    <h3 class="text-sm font-semibold text-slate-900 mb-2">Documentos requeridos (lista para el ciudadano)</h3>
                    <p class="text-xs text-slate-600 mb-3">Nombres de documentos que el ciudadano debe entregar.</p>
                ...
                <div class="flex items-center">
                    <input type="checkbox" name="activo" value="1" id="activo" {{ old('activo', $tipo->activo) ? 'checked' : '' }} class="h-4 w-4 rounded border-slate-300 text-slate-900 focus:ring-slate-900">
